...
//  Project1
//
//  Created by Xiaoran Liu on 10/1/21.
//
...
const char *WINDOWTITLE = { "OpenGL / Project 1 -- Xiaoran Liu" };
...
Display( )
{
    ...
    glColor3f( 0., 1., 1. );
    DoRasterString( 0., 1., 0., (char *)"This is a lamp." );
    ...
    glLoadIdentity( );
    glColor3f( 1., 1., 1. );
    DoRasterString( 5., 5., 0., (char *)"Xiaoran Liu - project 1" );
...
//cylinder
    glBegin(GL_QUADS);
        ...
        for(float i = 0.0; i < 2 * 3.14; i = i + 0.01){
            glVertex3f(0.1 * cos(i), 0.0, 0.1 * sin(i));
            glVertex3f(0.1 * cos(i+1), 0.0, 0.1 * sin(i+1));
            glVertex3f(0.1 * cos(i+1), 1.0, 0.1 * sin(i+1));
            glVertex3f(0.1 * cos(i), 1.0, 0.1 * sin(i));
        }
        glEnd();
