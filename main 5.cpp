...
const char *WINDOWTITLE = { "Project5 - Shaders" };
...
bool Freeze = 1;
int Distort;
bool VshaderOn = 1; //vertex shader is on
bool FshaderOn = 1; //fragment shader is on
#define MS_PER_CYCLE 70000
...
Display( )
{
    if( DebugOn != 0 )
      ...
      glShadeModel( GL_FLAT );
       ...
       gluLookAt( 0.5, 0.5, 2.5,     0., 0., 0.,     0., 1., 0. );
    ...
    Ds = 0.5 * cos(90*Time);
    Dt = 0.2 * sin(90*Time);
    Size =0.8 * sin(90*Time);

    ColorR = 0.9;
    ColorG = 0.7;
    ColorB = 0.5;

    SpecularColorR = 0.3 * cos(90*Time);
    SpecularColorG = 0.4 * cos(90*Time);
    SpecularColorB = 0.5 * cos(90*Time);
    Ka = 0.6;
    Kd = 0.8;
    Ks = 0.2;
    Shininess = 30.;
...
InitGraphics( )
{
    ...
    bool valid = Pattern->Create("/Users/xiaoranLiu/Desktop/project5/project5/patternlight.vert", "/Users/xiaoranLiu/Desktop/project5/project5/patternlight.frag");
    if (!valid)
    {
...
Keyboard( unsigned char c, int x, int y )
{
    ...
    switch( c )
    {
        ...
        case 'n': case 'N':
            ...
            //animate just the vertex shader pattern, freezing the fragment shader pattern
        case 'v': case 'V':
            VshaderOn = 1;
            FshaderOn = 0;
            break;
        
            //animate just the fragment shader pattern, freezing the vertex shader pattern
        case 'F':
            VshaderOn = 0;
            FshaderOn = 1;
            break;
         
            //animate both the vertex and fragment shader patterns together
        case 'b': case 'B':
            VshaderOn = 1;
            FshaderOn = 1;
            break;
            ...
            if( Distort )
                {
                    //...
                    p->s = (lng + M_PI) / (2.*M_PI);
                    p->t = (lat + M_PI/2. + cosf(2*M_PI*(Time + (float)ilng/(float)NumLngs)) ) / M_PI;
                }
            else
                {
                    p->s = ( lng + M_PI    ) / ( 2.*M_PI );
